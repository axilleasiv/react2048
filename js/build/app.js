...
var GridRow = React.createClass({displayName: 'GridRow',
	render: function() {
		...
		return (React.DOM.div( {className:"grid-row"}, 
					gridCells
				));
	}
});
...
var Grid = React.createClass({displayName: 'Grid',
	render: function(){
		...
		return (React.DOM.div(null, gridRows));
	}
});
...
var Tile = React.createClass({displayName: 'Tile',
	render: function(){
		...
		return (React.DOM.div( {style:tileStyle, className:tileClass}, 
					React.DOM.div( {className:"tile-inner"}, this.props.value)
				));
	}
});
...
var Tiles = React.createClass({displayName: 'Tiles',
	...
	render: function(){
		...
		return (React.DOM.div(null, tiles));
	}
});
...
var Button = React.createClass({displayName: 'Button',
	...
	render: function(){
		return (React.DOM.a( {onClick:this.handleClick, className:this.props.class}, this.props.text));
	}
});
...
var Score = React.createClass({displayName: 'Score',
	render: function(){
		return (React.DOM.div( {className:this.props.class}, this.props.score));
	}
});

var SelectTiles = React.createClass({displayName: 'SelectTiles',
	...
	render: function(){
		return (React.DOM.select( {'v-on':"change: changesTilesSize", onChange:this.handleChange}, 
					React.DOM.option( {value:"4"}, "4 tiles"),
					React.DOM.option( {value:"5"}, "5 tiles"),
					React.DOM.option( {value:"6"}, "6 tiles")
				));
	}
...
var Game = React.createClass({displayName: 'Game',
	...
	render: function(){
		...
		return (React.DOM.div( {className:"container", id:"mainVue"}, 
					React.DOM.div( {className:"heading"}, 
						React.DOM.h1( {className:"title"}, "2048"),
						React.DOM.div( {className:"scores-container"}, 
							Score( {class:"score-container", score:this.state.score} ),
							Score( {class:"best-container", score:this.state.bestScore} )
						)
					),
					React.DOM.div( {className:"above-game"}, 
						React.DOM.p( {className:"game-intro"}, "Join the numbers and get to the ", React.DOM.strong(null, "2048 tile!")),
						Button( {text:"New Game", class:"restart-button"} ),
						React.DOM.label( {className:"left", for:"noTiles"}, "Select the number of tiles"),
						SelectTiles(null )
					),
					React.DOM.div( {className:"game-container"}, 
						React.DOM.div( {className:"game-message"}, 
							React.DOM.p(null),
							React.DOM.div( {className:"lower"}, 
								React.DOM.a( {className:"keep-playing-button", 'v-on':"click: clearMessage"}, "Keep going"),
								React.DOM.a( {className:"retry-button", 'v-on':"click: init"}, "Try again")
							)
						),
						React.DOM.div( {className:"grid-container", id:"grid-container"}, 
							Grid(null)
						),
						React.DOM.div( {className:"tile-container", id:"tile-container"}, 
